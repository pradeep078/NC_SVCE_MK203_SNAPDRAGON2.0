  ...
  $.ajax({
    ...
    success : function(r) {
      ...
      if(r.error == 'NO_RECORD') {
        ...
        $('#output').html("<h1> Supervisor : "+r.teacher+"</h1><h4>Work-Site : "+$('input[name="code"]').val()+"</h4><h4>Total Days : "+r.count+"</h4><div id='chart'></div><div id='pie'></div>");
        generateGraph(r.timeline);
        $('#pie').html('<h2>Percentage : </h2> <input class="knob" value="'+(Math.floor(r.percent))+'" data-readonly="true" data-thickness=".4" readonly="readonly" data-width="150" data-height="150" data-angleOffset=180 data-fgColor="#87AB66" data-bgColor="#E1EAD9">');
        loadKnob();
        return;
      }
    },
